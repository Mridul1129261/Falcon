...
bool useForceField = false;   //true modified -MRIDUL
...
void updateHaptics(void)
{
    ...
    serverAddress.sin_port = htons(65345); // Use the same port number as the server
    serverAddress.sin_addr.s_addr = INADDR_ANY; // Replace with the server's IP address
    ...
    std::cout << "Server is listening on port 65345..." << std::endl;
...
// modification done
    ...
    while (simulationRunning)
    {
        ...
        while (true)
        {
            ...
            while (true) {
                int bytesReceived = recv(clientSocket, reinterpret_cast<char*>(&data), sizeof(haptics_data), 0);

                if (bytesReceived == sizeof(haptics_data)) {
                    duration++;
                }
                /*cout << "data # " << duration << endl;*/
                
                double Kp = 1.0; // [N/m]
                double Kv = 0.04433; // [Ns/m]
                
                hapticDevice->getPosition(position);
                hapticDevice->getLinearVelocity(linearVelocity);
                cVector3d forceField = (Kp * (data.position - position)) + (Kv * (data.velocity - linearVelocity));
                force.add(forceField);
                hapticDevice->getPosition(position);
                hapticDevice->getLinearVelocity(linearVelocity);
                /*cout << "position: " << data.position << endl;
                cout << "velocity: " << data.velocity << endl;*/
                hapticDevice->setForceAndTorqueAndGripperForce(force, torque, gripperForce);

            }
            
            /*if (3 > 5 && data.velocity.x() == 0.0 && data.velocity.y() == 0.0 && data.velocity.z() == 0.0)
            {
                hapticDevice->getPosition(position);
                forceField = (Kp * (data.position - position));
            }*/
            ...
            //    double Kp = 25; // [N/m]
            //    cVector3d forceField = Kp * (receivedPos - position);
            //    force.add(forceField);
